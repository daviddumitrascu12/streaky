(() => {
  ...
  function sanitize(input) {
    ...
    s.serverUrl = String(s.serverUrl || DEFAULT_STATE.serverUrl).trim().replace(/\/+$/, '') || DEFAULT_STATE.serverUrl;
  ...
  function createPanel() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        .buttons { display:grid; grid-template-columns: repeat(4, 1fr); border-top:1px solid rgba(255,255,255,.08); }
        ...
        .paused { opacity:.62; filter: grayscale(.25); }
      ...
      <div class="box" part="box">
        <div class="top" data-drag><div class="title">🔥 STREAK</div><div class="user" data-user>kein Name</div><button data-action="collapse">–</button></div>
        <div data-body>
          ...
          <div class="buttons"><button data-action="manual-correct">✓ +1</button><button data-action="manual-wrong">✕ reset</button><button data-action="pause">Pause</button><button data-action="sync">Sync</button></div>
        </div>
      </div>`;
  ...
  function updatePanel() {
    ...
    panel.status.textContent = state.username ? (state.paused ? 'Pausiert' : state.lastEventText) : 'Öffne das Add-on und wähle zuerst deinen Namen.';
    panel.sync.textContent = state.username ? state.lastSyncText : 'Leaderboard-Sync wartet auf Namen.';
    panel.box.classList.toggle('paused', !!state.paused);
  ...
  function onShadowClick(event) {
    ...
    if (action === 'collapse') {
      ...
      state.paused = !state.paused; state.lastEventText = state.paused ? 'Pausiert' : 'Automatik aktiv'; saveState(false);
    } else if (action === 'sync') {
      syncNow();
    } else if (action === 'manual-correct') {
      addAnswer(true, { key: `${normalizeQuestionHref(location.href)}#manual-${Date.now()}`, number: getNumberFromLocation() }, 'manuell'); saveState(true);
    } else if (action === 'manual-wrong') {
      addAnswer(false, { key: `${normalizeQuestionHref(location.href)}#wrong-${Date.now()}`, number: getNumberFromLocation() }, 'manuell'); saveState(true);
    }
  }
